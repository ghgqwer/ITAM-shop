...
export const parseTupleLiteral = (def, ctx) => {
    ...
    while (i < def.length) {
        ...
        if (spread) {
            ...
            sequences = sequences.map(base => appendElement(base, element.meta.optional ? "optional" : "required", element));
        }
    }
...
const parseDefaultTuple = (def, ctx) => ctx.$.parseOwnDefinitionFormat(def[0], ctx).default(def[2]);
const parseOptionalTuple = (def, ctx) => ctx.$.parseOwnDefinitionFormat(def[0], ctx).optional();
